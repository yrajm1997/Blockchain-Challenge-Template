...
func GenesisBlock(transactions []Transaction) Block {
	// TODO: Implement the CreateBlock function
	// 1. Create a new Block instance with the given parameters
	newBlock := Block{
		BlockID: 1,
...
func (b *Block) CalculateHash() string {
	...
	record := string(b.BlockID) + b.PreviousHash + b.Timestamp.String() + fmt.Sprintf("%v", b.Transactions)
...
func (bchain *Blockchain) AddNewBlock(transactions []Transaction) {
	if len(bchain.Blocks) == 0 {
		newBlock := GenesisBlock(transactions)
...
func ValidateBlockchainIntegrity(bchain Blockchain) bool {
	...
	// Validate PreviousHash of all blocks in blockchain
